...
def forensic_bot(query):
    ...
    if "math" in query or "integrity" in query:
        return (
            ...
            "and compares them against the final billed amount (Invoice Total)."
        )
...
# ----------------------------
# 6) Main Dashboard (RESTORED CHARTS & FILTERS)
# ----------------------------
else:
    with st.sidebar:
    ...
    if f:
        ...
        if not audit_df.empty:
            ...
            with col_f:
                ...
                options = audit_df["Category"].unique()
            ...
            st.divider()
            # RESTORED: Download Option
            csv = filtered_df.to_csv(index=False).encode('utf-8')
            st.download_button("Download Recovery Report (CSV)", data=csv, file_name="ClearSpend_Audit.csv")
