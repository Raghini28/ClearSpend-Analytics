...
/* Ensure visibility for chat messages */
...
def forensic_bot(query, audit_df):
    ...
    if "math" in query or "integrity" in query:
        return "**Math Integrity Check:** Verifies that the sum of line items matches the grand total."
    elif "total" in query:
...
# ----------------------------
    ...
    if f:
        ...
        if not audit_df.empty:
            ...
            with col_f:
                st.write("### 🔍 Findings")
                # Search filter is preserved here
            ...
            st.divider()
            # Download option is preserved here
